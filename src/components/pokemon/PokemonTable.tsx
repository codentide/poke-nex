      ...
      <td className="py-4 px-2">
        <div className="flex gap-2">
          {pokemon.types.map((type) => {
            const typeTheme = POKE_THEMES[type]
            return (
              <div key={type}>
                <div className={`${typeTheme.bg} p-2 rounded-full sm:hidden`}>
                  <TypeIcon type={type} size={14} className={typeTheme.text} />
                </div>
...
export const PokemonTable = ({ content }: Props) => {
  return (
    <div className="w-full overflow-x-auto rounded-xl border border-zinc-800 bg-zinc-900/25 backdrop-blur-sm">
